...
#         前端回傳結果
    ...
    def test_佮母語有關就回傳(self):
        對不起編號 = self.資料庫加外語('對不起')
        對不起華台編號 = self.華台對應(對不起編號, '拍謝', 'pháinn-sè')
        華台 = 華台對應表.揣編號(對不起華台編號)
        華台.提供正規化(self.pigu, 華台.使用者華語, '歹勢', 華台.使用者羅馬字)
#         前端輸入
        回應 = self.client.get('/平臺項目列表/揣列表', {
            '關鍵字': '歹'
        })
#         前端回傳結果
        self.assertEqual(回應.status_code, 200)
        回應資料 = 回應.json()
        self.assertEqual(len(回應資料['其他建議']), 1)

    def test_佮使用者提供ê漢字有關就回傳(self):
        對不起編號 = self.資料庫加外語('對不起')
        對不起華台編號 = self.華台對應(對不起編號, '拍謝', 'pháinn-sè')
        華台 = 華台對應表.揣編號(對不起華台編號)
        華台.提供正規化(self.pigu, 華台.使用者華語, '歹勢', 華台.使用者羅馬字)
...
#         前端回傳結果
    ...
    def test_佮使用者提供ê華語有關就回傳(self):
        對不起編號 = self.資料庫加外語('對步起')
        對不起華台編號 = self.華台對應(對不起編號, '歹勢', 'pháinn-sè')
        華台 = 華台對應表.揣編號(對不起華台編號)
        華台.提供正規化(self.pigu, '對不起',  華台.使用者漢字, 華台.使用者羅馬字)
#         前端輸入
        回應 = self.client.get('/平臺項目列表/揣列表', {
            '關鍵字': '對步起'
        })
