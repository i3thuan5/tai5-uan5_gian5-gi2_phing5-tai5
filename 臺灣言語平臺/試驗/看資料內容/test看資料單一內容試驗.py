...
# 		前端輸入
		回應 = self.client.get('/資料/單一內容', {'平臺項目編號':外語項目編號})
# 		前端回傳結果
...
# 		前端輸入
		回應 = self.client.get('/資料/單一內容', {'平臺項目編號':影音項目編號})
# 		前端回傳結果
		self.assertEqual(回應.status_code, 200)
		回應資料 = json.loads(回應.content.decode("utf-8"))
		self.assertEqual(回應資料, {
...
# 		前端輸入
		回應 = self.client.get('/資料/單一內容', {'平臺項目編號':文本項目編號})
# 		前端回傳結果
		self.assertEqual(回應.status_code, 200)
		回應資料 = json.loads(回應.content.decode("utf-8"))
		self.assertEqual(回應資料, {
